...
test_that("merge_shard_coverage overlays updates onto prior coverage, keeping untouched shards", {
  shard_2025 <- paste0(SHARD_PREFIX, "-2025.db")
  shard_2026 <- paste0(SHARD_PREFIX, "-2026.db")
  prev <- setNames(list(list(rows = 10L, date_min = "2025-01-01", date_max = "2025-12-31")), shard_2025)
  updates <- setNames(list(list(rows = 5L, date_min = "2026-01-01", date_max = "2026-06-30")), shard_2026)
  merged <- merge_shard_coverage(prev, updates)
  expect_equal(names(merged), c(shard_2025, shard_2026))
  expect_equal(merged[[shard_2025]]$rows, 10L)
...
test_that("write_release_notes renders the manifest summary, caveat, and shard coverage table", {
  ...
  manifest <- list(
    ...
    source_kind = "s3",
    changed_shards = as.list(shard_recent),
    shards = setNames(list(list(rows = 400L, date_min = "2025-06-01", date_max = "2026-06-30")), shard_recent),
    summary = list(packages = 3500L, latest_date = "2026-06-30"))
  path <- withr::local_tempfile(fileext = ".md")
  write_release_notes(path, manifest, RELEASE_CAVEAT)
  txt <- paste(readLines(path), collapse = "\n")
  expect_match(txt, "2026-06-30 05:00:00 UTC", fixed = TRUE)
  expect_match(txt, "3,500", fixed = TRUE)
  expect_match(txt, shard_recent, fixed = TRUE)
